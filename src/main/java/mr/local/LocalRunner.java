...
import mr.LocalOutputToMySqlFormat;
...
public class LocalRunner implements Tool {
    ...
    public static void main(String[] args) {
        try {
            ...
            logger.warn("Local TO MYSQL is failed !!!",e);
        }
    }
    @Override
...
//        conf.set(GlobalConstants.RUNNING_DATE,date);
        ...
        Job job = Job.getInstance(conf,"LocalTO MYSQL");

        job.setJarByClass(LocalRunner.class);

        //设置map相关参数
        job.setMapperClass(LocalMapper.class);
        job.setMapOutputKeyClass(StatusLocalDimension.class);
        job.setMapOutputValueClass(TextOutputValue.class);

        //设置reduce相关参数
        //设置reduce端的输出格式类
        job.setReducerClass(LocalReducer.class);
        job.setOutputKeyClass(StatusLocalDimension.class);
        job.setOutputValueClass(LocalOutputValue.class);

        job.setOutputFormatClass(LocalOutputToMySqlFormat.class);
